...
procedure GetEasyjetCHEAPPriceInfant(
            paramOutData: TStringList;
            returnflight : boolean;
            flexible: boolean;
            airport_depart: string;
...
begin
  ...
  begin
    GetEasyjetCHEAPPriceInfant( paramOutData,
            returnflight,
            flexible,
            airport_depart,
            IncMonth( EncodeDate(
...
// Don`t forget to insert   uses Forms;  above
    ...
    //May change 4 lines according to your rules
    ini := TIniFile.Create(IniPath);
    EasyjetStartURL  := ini.ReadString('main','EasyjetCHEAPStartURL', 'nil');
    ...
    if EasyjetStartURL='nil' then
    begin
      ShowMessage('Please chech the value of parameter EasyjetCHEAPStartURL in Easyjet.ini');
      exit; //please change the code according to your rules
    end;
//
    ParamWebBrowser.Height := 1;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if (Pos('|'+DateTimeToStringRet('myyyy',date_depart),Str) >0)
                or(Pos('|'+DateTimeToStringRet('mmyyyy',date_depart),Str) >0) then
              begin
          ...
          begin
            ...
            begin
              DocElement := DocSelect.Item(i,EmptyParam) as IHTMLElement;
              if pos('PRICE', UpperCase(DocElement.className)) > 0 then
              begin
                Str := DocElement.innerHTML;
                //ˆ
                if pos('ˆ', Str)>0 then
            ...
            price := currency_code+' '+FloatToStr(price_numeric*(adults+children)+price_infant*infants);

            // Get Flight Num / Time
            DocSelect := ((DocElementDiv.all as IHTMLElementCollection).tags('LI')as IHTMLElementCollection);
            if price_numeric > 0 then
            for i := 0 to DocSelect.length - 1 do
            begin
              DocElement := DocSelect.Item(i,EmptyParam) as IHTMLElement;
...
//
    ...
    begin
      paramWebBrowser.Document.QueryInterface(IHTMLDocument2, Doc);
      Str := Doc.cookie;
      if Pos('EJ_ORIG=', UpperCase(Str))>0 then
        Delete(Str, Pos('EJ_ORIG=', UpperCase(Str)), Length('EJ_ORIG')+5);
      if Pos('EJ_DEST=', UpperCase(Str))>0 then
        Delete(Str, Pos('EJ_DEST=', UpperCase(Str)), Length('EJ_DEST')+5);
      Str := Str+';eJ_orig='+airport_depart+';eJ_dest='+airport_destination;
      Doc.cookie := Str;
      Doc.Close;
    end;
    paramWebBrowser.Navigate('about:blank');
...
//          'orig='+airport_depart+'&'+
//          'dest='+airport_destination+'&'+
//          'oDay='+DateTimeToStringRet('dd',loc_date_depart+1)+'&'+
//          'oMonYear='+DateTimeToStringRet('mm',loc_date_depart+1)+''+
//                      DateTimeToStringRet('yyyy',loc_date_depart+1)+'&'+
//          'rDay='+DateTimeToStringRet('dd',loc_date_return+1)+'&'+
//          'rMonYear='+DateTimeToStringRet('mm',loc_date_return+1)+''+
