...
procedure WriteToFile(FN:string;Cntr:integer;S:widestring);
...
begin
// set controls...
if paramWebBrowser.Document <> nil
then
  begin
  ...
  for i := 0 to DocSelect.length - 1 do
    ...
    if not paramChangeDirections
    then
      begin
      // destination
      if POS('ORIGIN',UpperCase(DocElementSelect.name))>0 then DocElementSelect.value := airport_depart;
      if POS('DESTIN',UpperCase(DocElementSelect.name))>0 then
        begin
          try
          DocElementSelect.add((Doc.parentWindow.Option as IHTMLOptionElementFactory).create(airport_destination,airport_destination,true,true) as IHTMLElement, true);
          except
          end;
      ...
      begin
      // destination
      if POS('ORIGIN',UpperCase(DocElementSelect.name))>0 then  DocElementSelect.value := airport_destination;
      if POS('DESTIN',UpperCase(DocElementSelect.name))>0 then
        begin
        try
        DocElementSelect.add((Doc.parentWindow.Option as IHTMLOptionElementFactory).create(airport_depart,airport_depart,true,true) as IHTMLElement, true);
        except
        end;
    ...
    // persons
    if POS('NUMADT',UpperCase(DocElementSelect.name))>0 then DocElementSelect.value := IntToStr(adults);
    if POS('NUMCHD',UpperCase(DocElementSelect.name))>0 then DocElementSelect.value := IntToStr(children);
    if POS('NUMINF',UpperCase(DocElementSelect.name))>0 then DocElementSelect.value := IntToStr(infants);
    // dates
    if not paramChangeDirections then
      begin
      if POS('DAY0',UpperCase(DocElementSelect.name))>0 then DocElementSelect.value := DateTimeToStringRet('dd',loc_date_depart);
      if POS('DAY1',UpperCase(DocElementSelect.name))>0 then DocElementSelect.value := DateTimeToStringRet('dd',loc_date_return);
      if POS('MONTH0',UpperCase(DocElementSelect.name))>0 then DocElementSelect.value := DateTimeToStringRet('mm',loc_date_depart)+
                                                                                   '-'+DateTimeToStringRet('yyyy',loc_date_depart);
      if POS('MONTH1',UpperCase(DocElementSelect.name))>0 then DocElementSelect.value := DateTimeToStringRet('mm',loc_date_return)+
                                                                                   '-'+DateTimeToStringRet('yyyy',loc_date_return);
      end
    else
      begin
      if POS('DAY0',UpperCase(DocElementSelect.name))>0 then DocElementSelect.value := DateTimeToStringRet('dd',loc_date_return);
      if POS('DAY1',UpperCase(DocElementSelect.name))>0 then DocElementSelect.value := DateTimeToStringRet('dd',loc_date_return+1);
      if POS('MONTH0',UpperCase(DocElementSelect.name))>0 then DocElementSelect.value := DateTimeToStringRet('mm',loc_date_return)+
                                                                                   '-'+DateTimeToStringRet('yyyy',loc_date_return);
      if POS('MONTH1',UpperCase(DocElementSelect.name))>0 then DocElementSelect.value := DateTimeToStringRet('mm',loc_date_return+1)+
                                                                                  '-'+DateTimeToStringRet('yyyy',loc_date_return+1);
      end;
    // travel class
    if POS('COMPARTMENT',UpperCase(DocElementSelect.name))>0 then DocElementSelect.value := 'ER';
    except
    end;
  ...
  for i := 0 to DocSelect.length - 1 do
    ...
    if POS('TRAVELTYPE',UpperCase(DocElementInput.name))>0 then
      if POS('bti',UpperCase(DocElement.innerHTML))>0 then DocElementInput.checked := true;
    if POS('LEGS',UpperCase(DocElementInput.name))>0 then
      begin
        if paramIsReverse then DocElementInput.checked := DocElementInput.value = '2'
        else DocElementInput.checked := DocElementInput.value = '1';
      end;
    except
    end;
  // Click "Select"
  ...
  for i := 0 to DocSelect.length - 1 do
    ...
    if POS('CONTINUE',UpperCase(DocElementInput.value))>0 then
      ...
      //if DocElementInput.form <> NULL then
        if POS('FB_INPUT',UpperCase(DocElementInput.form.name))>0 then   DocElement.click;
    except
    end;
...
if (paramWebBrowser.Document <> nil) and (DocElementResults <> nil) then
  ...
  if flexible then
    ...
    if Pos('forward',flight_type)>0 then
      if (loc_date_depart < date_depart) or (loc_date_depart>date_return) then flight_type := '';
    if Pos('reverse',flight_type)>0 then
      if (loc_date_return < date_depart) or (loc_date_return>date_return) then flight_type := '';
    end
  else
    begin
    if Pos('forward',flight_type)>0 then
      if (loc_date_depart <> date_depart) and (loc_date_depart<>date_return) then  flight_type := '';
    if Pos('reverse',flight_type)>0 then
      if (loc_date_return <> date_depart) and (loc_date_return<>date_return) then  flight_type := '';
    end;
  ...
  for div_index := 0 to DocSelectDiv.length - 1 do
    ...
    WriteToFile('File.html',Cntr,DocElementDiv.innerHTML);
    Inc(cntr);
    if ( Pos('ID_FLTDATE_', UpperCase(DocElementDiv.id))>0 ) then
      ...
      if paramChangeDirections then  direction_forward := not direction_forward;
      end
    else
    StrHTML := '';
    ...
    //    Str := '<'
    //   else
    if direction_forward then  Str := DateTimeToStringRet('mmm',loc_date_depart)+DateTimeToStringRet('d',loc_date_depart)
    else Str := DateTimeToStringRet('mmm',loc_date_return)+DateTimeToStringRet('d',loc_date_return);
        ...
        for td_index := 0 to DocSelectTD.length - 1 do
          ...
          if( Pos('ID_FLT_', UpperCase(DocElementTD.id))>0 ) then
            ...
            while (Pos('<A HREF="JAVASCRIPT:OPENPOP(', UpperCase(strHTML)) > 0) do
              ...
              while Pos('>', UpperCase(Str)) >0 do  Delete(Str,1, Pos('>', UpperCase(Str)));
              Str := Trim(Str);
              //(+1)
              if (Str[1] = '(') and (Pos(')', Str)<=6) then  Delete(Str,1, Pos(')', Str));
              Str := Trim(Str);
              if Pos(' ', UpperCase(Str)) > 0 then
                begin
                if flight_no = '' then  depart_time := Copy(Str,1,Pos(' ', UpperCase(Str))-1);
                Delete(Str,1,Pos(' ', UpperCase(Str)));
                end;
              Str := Trim(Str);
              if Pos(':', UpperCase(Str)) > 0 then
                begin
                if flight_no <> '' then  via := Trim(Copy(Str,1,Pos(':', UpperCase(Str))-3));
              ...
              if Pos('>', strHTML) >0 then Delete(strHTML,1, Pos('>', strHTML));
              if Pos('<', strHTML) >0 then
                begin
                Str := Copy(strHTML,1,Pos('<', UpperCase(strHTML))-1);
                if TryStrToInt(Str, j) then Str := default_flight_suffix + Str;
                if flight_no <> '' then flight_no := flight_no + ',';
                flight_no := flight_no + Trim(Str);
                end;
              end;
            // flight duration
            flight_duration := '';
            if Pos('FLIGHT DURATION:', UpperCase(strHTML)) >0 then  Delete(strHTML, 1, Pos('FLIGHT DURATION:', UpperCase(strHTML)) );
            if Pos('>', strHTML) >0 then Delete(strHTML,1, Pos('>', strHTML));
            ...
            if TryStrToInt(copy(depart_time,4,2),depart_time_m) then
              begin
              if (depart_time_m + flight_duration_m)>=60 then Inc(flight_duration_h);
              if (depart_time_h + flight_duration_h)>=24 then
                begin
                  if direction_forward
                  then
                  arrival_date := DateTimeToStringRet('dd mmm yyyy',loc_date_depart+(depart_time_h + flight_duration_h) div 24)
                  else
                  arrival_date := DateTimeToStringRet('dd mmm yyyy',loc_date_return+(depart_time_h + flight_duration_h) div 24);
                end;
              end;
            ...
            then
               strl_forward.add(
                        FloatToStr(loc_date_depart)+depart_time+'|'+ depart_date+'|'+ depart_time+'|'+ arrival_date+'|'
                                                                                   + arrival_time+'|'+flight_no+'|'+ via)
            else
                  strl_return.add(FloatToStr(loc_date_depart)+depart_time+'|'+depart_date+'|'+depart_time+'|'+arrival_date
                                                                                 +'|'+arrival_time+'|'+flight_no+'|'+via);
            end;
          end;
        end;
  ...
  while strl_forward.count > strl_return.count do
    if strl_return.Count > 0
    then  strl_return.Add(strl_return[strl_return.Count-1])
    else strl_return.Add(' | | | | | | ');
  while strl_return.count > strl_forward.count do
    if strl_forward.Count > 0
    then  strl_forward.Add(strl_forward[strl_forward.Count-1])
    else strl_forward.Add(' | | | | | | ');
  Result := strl_forward.Count;
  // Get Price
  if paramIsReverse then
    begin
    if flexible
    then Str := 'ID_MATRIX_'+IntToStr(round(loc_date_depart - date_depart) mod 7)+'_'+IntToStr(6 - (round(date_return-loc_date_return) mod 7))
    else Str := 'ID_MATRIX_3_3';
    end
  else
    begin
    if flexible then
      begin
      if direction_forward
      then  Str := 'ID_MATRIX_'+IntToStr(round(loc_date_depart - date_depart) mod 7)
      else  Str := 'ID_MATRIX_'+IntToStr(6 - (round(date_return-loc_date_return) mod 7))
      end
    else  Str := 'ID_MATRIX_3';
    end;
  ...
  for div2_index := 0 to DocSelectDiv2.length - 1 do
    ...
    if Price <> '' then
      ...
      for I := 0 to strl_return.Count - 1 do
        ...
        Delete(Str,1,pos('|',Str));
        strl_return[i] := str;
        paramOutData.add(FormatFloat('000',Length(flight_type))+flight_type+ strHTML
              +'|'+flight_type+'|'+strl_forward[i]+'|'+strl_return[i]+'|'+currency_code+' '+price+'|');
          end;
      end;
    end;
  Doc.close;
...
if loc_date_return<loc_date_depart then loc_date_return := loc_date_depart;
...
for day_index := 0 to 6 do
  ...
  if flight_per_day_max < flight_per_day_curr
  then
...
then
  ...
  if loc_date_return<loc_date_depart
  then
  ...
  for day_index := 0 to 6 do
    ...
    if flight_per_day_max < flight_per_day_curr then flight_per_day_max := flight_per_day_curr;
...
begin
  try
  AssignFile(F,'Webpage\'+IntToStr(Cntr)+FN);
  Rewrite(F);
  Writeln(F,S);
  finally
  CloseFile(F);
 ...
 procedure ExecuteScript(doc: IHTMLDocument2; script: string) ;
 begin
    if doc <> nil then
    begin
...
//      script:='openpop(''fb.fly?action=details&flt=0,0,0'', '''', '''', ''leg0'')';
//      if doc.parentWindow <> nil then doc.parentWindow.ExecScript(script, Olevariant('javascript'));
