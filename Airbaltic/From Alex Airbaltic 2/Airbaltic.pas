...
//  price_sum,price_numeric : double;
  ...
  procedure GetResultsForDay(paramIsReverse : boolean);
  var
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if paramIsReverse and (Pos('ID_FLTDATE_1', UpperCase(DocElementDiv.id))<=0) then
            StrHTML := '';
        ...
        begin
          ...
          begin
            if flexible then
              Str := 'ID_MATRIX_'+IntToStr(round(loc_date_depart - date_depart) mod 7)+'_'+IntToStr(round(loc_date_depart - date_depart) mod 7)
             else
              Str := 'ID_MATRIX_3_3';
          end
          else
...
//              Str := 'ID_MATRIX_'+IntToStr(round(loc_date_depart - date_depart) mod 7);
          ...
          if paramIsReverse and TryStrToFloat(price, price_2way) then
          begin
            price := '';
            // search forward flight
            for I := 0 to paramOutData.Count - 1 do
            if pos('|forward|', paramOutData[i]) > 0 then
            begin
              Str := paramOutData[i];
              Delete(str,1,pos('|',str));
              Delete(str,1,pos('|',str));
              if pos(DateTimeToStringRet('dd mmm yyyy',loc_date_depart), Str) > 0 then
              begin
                Delete(str,1,pos('|',str));
                Delete(str,1,pos('|',str));
                Delete(str,1,pos('|',str));
                ...
                Str := Trim(Str);
                if TryStrToFloat(Str, price_forward) then
                  if price_2way > price_forward then
                    price := FloatToStr(price_2way-price_forward);
              end;
            end;
          end;
