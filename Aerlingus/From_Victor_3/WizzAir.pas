...
begin
  ...
  try
    ...
    if WizzAirStartURL = 'nil' then begin
    ...
    while (loc_date_depart <= date_return) do begin
      ...
      if not Assigned(paramWebBrowser.Document) then begin
...
begin
  ...
  if Assigned(Doc) then begin
    try
      ...
      for i := 0 to DocCollection.length-1 do begin
        Element := (DocCollection.item(i, 0) as IHTMLElement);
        if Element.getAttribute('id', 0) = 'mcpPriceChange' then begin
          cb := (Element as IHTMLSelectElement);
          for k := 0 to cb.length-1 do begin
            if (cb.item(k, 0) as IHTMLElement).innerText = Currency then begin
              cb.selectedIndex := k;
              if Assigned(Doc.parentWindow) then begin
                Doc.parentWindow.ExecScript('updateMcpTooltip()', OleVariant('JavaScript')) ;
                html := wb.OleObject.Document.documentElement.innerHTML;
                if Pos('AllInclusivePriceSummary_summary', html) > 0 then begin
                ...
                if Pos('mcpPriceFinal', html) > 0 then begin
